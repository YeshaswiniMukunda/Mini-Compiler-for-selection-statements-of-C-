...
Objectstmt : t_ID obj ';';
obj : t_ID ',' obj {strcpy(sym[$1].prop,"OBJECT");}| t_ID {strcpy(sym[$1].prop,"OBJECT");}
...
void main()
{
yyin=fopen("file.cpp","r+");
yyout=fopen("out.txt","w");
yyparse();
int i;
for(i=0;i<count;i++){
printf("index: %2d, name: %10s, property: %3s, type: %d, value : %s, line_no: %d ,prev: %d , scope: %s\n",i,sym[i].name,sym[i].prop,sym[i].type,sym[i].value,sym[i].line_no,sym[i].lat_seen, sym[i].scope);
}
